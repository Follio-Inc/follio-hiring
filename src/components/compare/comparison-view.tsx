...
export function ComparisonView({ applications, jobId, onClose }: ComparisonViewProps) {
  ...
  return (
    <div className="space-y-6">
      {/* AI Insight */}
      <div className="p-5 bg-gradient-to-r from-violet-50/80 via-purple-50/60 to-indigo-50/40 border border-violet-100/60 rounded-2xl animate-fade-in-up" style={{ animationDelay: '0ms' }}>
        <div className="flex items-center gap-2 mb-2">
          <Sparkles size={16} className="text-violet-600" />
          <h3 className="text-sm font-bold text-violet-900">AI Comparison Insight</h3>
        </div>
        <p className="text-sm text-violet-800/80 leading-relaxed">{insight}</p>
      </div>
      ...
      <div className="grid gap-4 animate-fade-in-up" style={{ gridTemplateColumns: `200px repeat(${applications.length}, 1fr)`, animationDelay: '80ms' }}>
        ...
        {applications.map(app => (
          <div key={app.id} className="text-center p-5 bg-white border border-stone-200/80 rounded-2xl hover:shadow-lg hover:shadow-stone-200/30 transition-all duration-300">
      ...
      {/* Comparison Table */}
      <div className="bg-white border border-stone-200/80 rounded-2xl overflow-hidden animate-fade-in-up" style={{ animationDelay: '160ms' }}>
        {dimensions.map((dim, idx) => (
          <div
            key={dim.label}
            className={cn(
              'grid items-center py-3.5 px-5',
              idx !== dimensions.length - 1 && 'border-b border-stone-100'
            )}
      ...
      {/* Skills Comparison */}
      <div className="bg-white border border-stone-200/80 rounded-2xl p-5 animate-fade-in-up" style={{ animationDelay: '240ms' }}>
      ...
      <div className="grid gap-4 animate-fade-in-up" style={{ gridTemplateColumns: `repeat(${applications.length}, 1fr)`, animationDelay: '320ms' }}>
        {applications.map(app => (
          <div key={app.id} className="bg-white border border-stone-200/80 rounded-2xl p-5">
            <p className="text-xs font-semibold text-stone-400 mb-3">{app.candidate.name}</p>

            <div className="mb-4">
              <h5 className="text-xs font-bold text-emerald-700 flex items-center gap-1 mb-2">
